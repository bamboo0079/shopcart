...
class ControllerSaleCustomer extends Controller { 
	...
	public function update_group() {
		...
		$array_mienbac = array(3752,3753,3755,3764,3768,3772,3773,3774,3775,3776,3779,3782,4036,4038,4039,4041,4043,4045,4049,4052,4054,4055,4060,4062,4063);
		$array_mientrung = array(3758,3761,3765,3766,3767,3771,3778,4033,4035,4037,4042,4044,4046,4047,4048,4050,4056,4057,4064);
		$array_miennam = array(3751,3756,3754,3757,3759,3760,3762,3763,3769,3770,3781,3780,4034,4040,4051,4053,4058,4059,4061);
		
		$results = $this->model_sale_customer->getCustomers();
		
		foreach ($results as $result) {
			$address = $this->model_sale_customer->getAddress($result['address_id']);
			if(in_array($address['zone_id'],$array_mienbac) || in_array($address['zone_id'],$array_mientrung) || in_array($address['zone_id'],$array_miennam)){
				if(in_array($address['zone_id'],$array_mienbac)){
					$this->model_sale_customer->updateGroupByCId($result['customer_id'],2);
				}elseif(in_array($address['zone_id'],$array_mientrung)){
					$this->model_sale_customer->updateGroupByCId($result['customer_id'],3);
				}elseif(in_array($address['zone_id'],$array_miennam)){
					$this->model_sale_customer->updateGroupByCId($result['customer_id'],4);
				}
				$this->session->data['success'] = $this->language->get('text_success');
			}else{
				$this->model_sale_customer->updateGroupByCId($result['customer_id'],5);
				$this->session->data['success'] = $this->language->get('text_success');
			}
		}
		$this->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'] . $url, 'SSL'));
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_customer_type_id'])) {
			$filter_customer_type_id = $this->request->get['filter_customer_type_id'];
		} else {
			$filter_customer_type_id = null;
		}
		...
		if (isset($this->request->get['filter_customer_type_id'])) {
			$url .= '&filter_customer_type_id=' . $this->request->get['filter_customer_type_id'];
		}
		...
		$this->data['update_group'] = $this->url->link('sale/customer/update_group', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data = array(
			...
			'filter_customer_type_id' => $filter_customer_type_id, 
		...
		$this->data['column_customer_type'] = $this->language->get('column_customer_type');
		...
		$this->data['button_update_group'] = $this->language->get('button_update_group');
		...
		$this->data['sort_customer_type'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=customer_type' . $url, 'SSL');
		...
		$this->data['filter_customer_type_id'] = $filter_customer_type_id;
		...
		$this->data['customer_types'] = $this->model_sale_customer_type->getCustomerTypes();
	...
	protected function getForm() {
		...
		$this->data['entry_send_mail'] = $this->language->get('entry_send_mail');
		...
		if (isset($this->request->post['customer_type_id'])) {
			$this->data['customer_type_id'] = $this->request->post['customer_type_id'];
		} elseif (!empty($customer_info)) {
			$this->data['customer_type_id'] = $customer_info['customer_type_id'];
		} else {
			$this->data['customer_type_id'] = '';
		}
		...
		if (isset($this->request->post['send_mail'])) {
			$this->data['send_mail'] = $this->request->post['send_mail'];
		} else {
			$this->data['send_mail'] = 1;
		}
