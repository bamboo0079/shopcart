...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data = array(
			...
			'filter_date'     		 => datedata_to_date($filter_date),
		...
		$this->data["results_user"] = $this->model_user_user->getNameUsers($user_group_id);
		$results = $this->model_sale_order->getOrdersByUser($data);
		...
		foreach ($results as $result) {
			...
			if($result['order_status_id'] == 5){
				$action[] = array(
					...
					'target' => '_blank',
					'href' => '/order/view/invoice'.'?order_id=' . $this->encryption->encrypt($result['order_id'])
				);
				$action[] = array(
					'text' => $this->language->get('text_confirm'),
					'target' => '_blank',
					'href' => '/order/view/confirm'.'?order_id=' . $this->encryption->encrypt($result['order_id'])
				);
			}
			$action[] = array(
			...
			if($result['order_status_id']==2){
				//Đang xử lý
				$order_status_id = sprintf($this->language->get('text_order_status_id2'),$result['status']);
			}elseif($result['order_status_id']==1){
				//Chưa xử lý
				$order_status_id = sprintf($this->language->get('text_order_status_id1'),$result['status']);
			}elseif($result['order_status_id']==7){
				//Đã hủy bỏ
				$order_status_id = sprintf($this->language->get('text_order_status_id7'),$result['status']);
			}elseif($result['order_status_id']==5){
				//Hoàn tất
				$order_status_id = sprintf($this->language->get('text_order_status_id5'),$result['status']);
			}else{
			...
			//first log
			$log = '';
			$logs = $this->model_sale_order->getOrderLogs($result['order_id']);	
			
			if($logs){
				$first_log = array_pop($logs);
				$log = $first_log['user'];
			}
			//option date
			$options = $this->model_sale_order->getOrderOptionsByOrderID($result['order_id']);
			$orders = 	array(
			...
			if (isset($this->request->get['filter_order_user'])) {
				if ($this->request->get['filter_order_user'] == $orders['user']) {
		...
		$this->data['column_manager'] = $this->language->get('column_manager');
		...
		$this->data['sort_date'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.date' . $url, 'SSL');
	...
	public function getForm() {
		...
		if(isset($order_info['order_status_id'])){
			if($order_info['order_status_id'] == 5){
				$this->data['invoice'] = '/order/view/invoice'.'?order_id=' . $this->encryption->encrypt($this->request->get['order_id']);
				$this->data['confirm'] = '/order/view/confirm'.'?order_id=' . $this->encryption->encrypt($this->request->get['order_id']);
			}
		}
		...
		$this->data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();	
		//var_dump($this->data['order_statuses']);
		...
		foreach ($order_products as $order_product) {
			...
			$this->data['order_products'][] = array(
				...
				'duration'            => $order_product['duration'],
				'start_time'            => $order_product['start_time'],
				'meeting'            => $order_product['meeting'],
				'included'            => $order_product['included'],
				'notincluded'            => $order_product['notincluded'],
				'terms'            => $order_product['terms'],
				'option'           => $order_option,
				'download'         => $order_download,
				'quantity'         => $order_product['quantity'],
				'price'            => $order_product['price'],
				'total'            => $order_product['total'],
				'price_text'            => $this->currency->format($order_product['price']),
				'total_text'            => $this->currency->format($order_product['total']),
		...
		//DEPOSIT UPDATE
		$this->data['deposit'] = $this->data['balance'] = $this->data['note_deposit'] = $this->data['status_deposit'] = '';
		if(isset($this->request->get['order_id'])){
			$deposit = $this->model_sale_order->getOrderDeposit($this->request->get['order_id']);			
			if($deposit){
				$this->data['deposit'] = $deposit['deposit'];
				$this->data['balance'] = $deposit['balance'];
				$this->data['note_deposit'] = $deposit['note'];
				$this->data['status_deposit'] = $deposit['status'];
			}
		}
		
		//LOG UPDATE
		$this->data['array_admin'] = array(1,14,31);
		$this->data['logs'] = array();
		if(isset($this->request->get['order_id'])){
			$logs = $this->model_sale_order->getOrderLogs($this->request->get['order_id']);	
			if($logs){
		...
		if (isset($this->request->post['order_total']) && !isset($this->request->get['order_id'])) {
			...
			$totalsaleoff = $this->model_sale_order->getTotalSaleOff($this->request->get['order_id']);

			if($totalsaleoff){

				$this->data['order_totals'][0]['saleoff'] = $this->currency->format($totalsaleoff['saleoff']);
				$this->data['order_totals'][1]['text'] = $this->currency->format($totalsaleoff['total_saleoff']);
	...
	/* Khoa: kiem tra gan nhan vien cho don hang*/

	public function getUserNameByOrder($order_id) {
		if ($this->user->getUserGroupID() == 11) {
