...
class ControllerCatalogNews extends Controller {
	private $error = array(); 
  	...
  	private function getList() {
		...
		$this->data['array_admin'] = array(2,31);
		...
		foreach ($results as $result) {
			...
			$log = '';
			$date_log = '';
			$logs = $this->model_catalog_news->getNewsLogs($result['news_id']);	
			
			if($logs){
				$first_log = array_shift($logs);
				$log = $first_log['user'];
				$date_log = $first_log['date'];
			}
			//
		...
		$categories = $this->model_catalog_news->getNewsCategories($result['news_id']);
		$cate_name = '';
		
		foreach($categories as $category){
			
			$rs = $this->model_catalog_news_category->getNewsCategoryDescriptions($category);
			foreach($rs as $rs){
				$cate_name .= '- '.$rs['name'].'</br>';
			}
		}	
		$keyword = $this->model_catalog_news->getNews($result['news_id']);
      	$this->data['news'][] = array(
  	...
  	private function getForm() {
		...
		$this->data['entry_price_sale'] = $this->language->get('entry_price_sale');
		$this->data['entry_special_offer'] = $this->language->get('entry_special_offer');
		...
		$this->data['entry_highlight'] = $this->language->get('entry_highlight');		
		...
		$this->data['entry_note_info'] = $this->language->get('entry_note_info');		
		$this->data['entry_termns'] = $this->language->get('entry_termns');	
		...
		if (isset($this->request->get['news_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		...
      		$logs = $this->model_catalog_news->getNewsLogs($this->request->get['news_id']);
			$this->data["logs"] = $logs;
    	}
		...
		$this->data['types'] = $this->model_catalog_news_type->getNewsTypes(0);
		
		if (isset($this->request->post['news_type'])) {
			$this->data['news_type'] = $this->request->post['news_type'];
		} elseif (isset($news_info)) {
			$this->data['news_type'] = $this->model_catalog_news->getNewsTypes($this->request->get['news_id']);
		} else {
