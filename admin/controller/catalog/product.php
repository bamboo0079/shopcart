...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_product_class'])) {
			$filter_product_class = $this->request->get['filter_product_class'];
		} else {
			$filter_product_class = null;
		}
		...
		if (isset($this->request->get['filter_product_class'])) {
			$url .= '&filter_product_class=' . urlencode(html_entity_decode($this->request->get['filter_product_class'], ENT_QUOTES, 'UTF-8'));
		}
		...
		foreach ($results as $result) {
			...
			if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
				$image = $this->model_tool_image->onesize($result['image'], 40, 40);
			} else {
				$image = $this->model_tool_image->onesize('no_image.jpg', 40, 40);
			}
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || $product_special['date_start'] < date('Y-m-d')) && ($product_special['date_end'] == '0000-00-00' || $product_special['date_end'] > date('Y-m-d'))) {
					$special = number_format($product_special['price'], 0, ",", ".");

					break;
				}					
			}
			$category = array();
			$cates = $this->model_catalog_product->getProductCategories($result['product_id']);
			if($cates){
				foreach($cates as $cate){
					$categories = $this->model_catalog_category->getCategory($cate);
					$category[] = array(
			...
			$keyword = $this->model_catalog_product->getProduct($result['product_id']);
			if(isset($keyword['keyword'])){
				$custom_link = $this->model_catalog_product->getCustomelink($result['custom_link']);
				if(isset($custom_link['keyword'])){
					$this->data['products'][] = array(
						...
						'model'      => $result['model'],
						'keyword'		=> $custom_link['keyword'].'/'.$keyword['keyword'],
						...
						'action'     => $action,
						'hrefedit' 	 => $this->url->link('catalog/product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
					);
				}else{
					$this->data['products'][] = array(
						'product_id' => $result['product_id'],
		...
		$this->data['text_group_tour'] = $this->language->get('text_group_tour');	
		$this->data['text_private_tour'] = $this->language->get('text_private_tour');		

		$this->data['column_image'] = $this->language->get('column_image');		
		$this->data['column_name'] = $this->language->get('column_name');		
		$this->data['column_category'] = $this->language->get('column_category');
		$this->data['column_product_class'] = $this->language->get('column_product_class');			
		...
		$this->data['sort_category'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=pd.category' . $url, 'SSL');
		$this->data['sort_product_class'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.product_class' . $url, 'SSL');
		...
		$this->data['filter_product_class'] = $filter_product_class;
	...
	protected function getForm() {
		...
		$this->data['entry_name_tour'] = $this->language->get('entry_name_tour');
		...
		$this->data['entry_delay_book'] = $this->language->get('entry_delay_book');
		$this->data['entry_robot_index'] = $this->language->get('entry_robot_index');
		$this->data['entry_custom_breadcrumb'] = $this->language->get('entry_custom_breadcrumb');
		$this->data['entry_custom_link'] = $this->language->get('entry_custom_link');
		...
		$this->data['entry_product_suggest'] = $this->language->get('entry_product_suggest');
		...
		$this->data['button_add_product_menu'] = $this->language->get('button_add_product_menu');
		...
		$this->data['button_add_product_detail'] = $this->language->get('button_add_product_detail');
		$this->data['tab_product_detail'] = $this->language->get('tab_product_detail');
		$this->data['tab_product_menu'] = $this->language->get('tab_product_menu');
		...
		if (isset($this->request->post['robot_index'])) {
			$this->data['robot_index'] = $this->request->post['robot_index'];
		} elseif (!empty($product_info)) {
			$this->data['robot_index'] = $product_info['robot_index'];
		} else {
		...
		if (isset($this->request->post['product_class'])) {
			$this->data['product_class'] = $this->request->post['product_class'];
		} elseif (!empty($product_info)) {
			$this->data['product_class'] = $product_info['product_class'];
		} else {
			$this->data['product_class'] = '';
		}
		//UPDATE TU CHON URL DANH MUC
		$cate_id = '';
		if(isset($this->request->get['product_id'])){
			$cate_id = $this->model_catalog_product->getProductCategories($this->request->get['product_id']);
			if (isset($cate_id[2])) {
				$cate_id = $cate_id[2];
		...
		if (isset($this->request->post['custom_breadcrumb'])) {
			$this->data['custom_breadcrumb'] = $this->request->post['custom_breadcrumb'];
		} elseif (isset($product_info) && $product_info['custom_breadcrumb'] > 0) {
			$this->data['custom_breadcrumb'] = $product_info['custom_breadcrumb'];
		} else {
			$this->data['custom_breadcrumb'] = $cate_id;
		}
		
		if (isset($this->request->post['custom_link'])) {
			$this->data['custom_link'] = $this->request->post['custom_link'];
		} elseif (isset($product_info) && $product_info['custom_link'] > 0) {
			$this->data['custom_link'] = $product_info['custom_link'];
		} else {
			$this->data['custom_link'] = $cate_id;
		}
		...
		if (isset($this->request->post['delay_book'])) {
			$this->data['delay_book'] = $this->request->post['delay_book'];
		} elseif (!empty($product_info)) {
			$this->data['delay_book'] = $product_info['delay_book'];
		} else {
			$this->data['delay_book'] = '2';
		}
		...
		if (isset($this->request->post['schedule'])) {
			$this->data['schedule'] = $this->request->post['schedule'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['sub_duration'])) {
			$this->data['sub_duration'] = $this->request->post['sub_duration'];
		} elseif (!empty($product_info)) {
			$this->data['sub_duration'] = $product_info['sub_duration'];
		} else {
			$this->data['sub_duration'] = '';
		}
		...
		if (isset($this->request->post['image']) && file_exists(DIR_IMAGE . $this->request->post['image'])) {
			$this->data['thumb'] = $this->model_tool_image->onesize($this->request->post['image'], 100, 100);
		} elseif (!empty($product_info) && $product_info['image'] && file_exists(DIR_IMAGE . $product_info['image'])) {
			$this->data['thumb'] = $this->model_tool_image->onesize($product_info['image'], 100, 100);
		} else {
			$this->data['thumb'] = $this->model_tool_image->onesize('no_image.jpg', 100, 100);
		}
		...
		if (isset($this->request->post['policy'])) {
            ...
            $this->data['policy'] = $product_info['policy'];
        } else {
            $this->data['policy'] = 1;
        }
        $this->data['policies'] = array();
        $this->load->model('catalog/policy');
        $policies = $this->model_catalog_policy->getPolicys();
        foreach ($policies as $item) {
            $cate                     = ($item['category'] == 0) ? 'Tour ghép đoàn' : 'Tour riêng';
            $this->data['policies'][] = array(
                'id' => $item['policy_id'],
                'name' => $item['name'],
                'category' => $cate,
		...
		$this->load->model('catalog/attribute_type');
		
		$this->data['attribute_types'] = array();
		$attribute_type_info = $this->model_catalog_attribute_type->getAttributeTypes();
		if ($attribute_type_info) {
			foreach ($attribute_type_info as $result) {
			$this->data['attribute_types'][] = array(
				'attribute_type_id'                  => $result['attribute_type_id'],
				'name'                          		=> $result['name']
				);
			}
		}	
			...
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				foreach ($product_option['product_option_value'] as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'include'                  => $product_option_value['include']
					);
				}
		...
		foreach ($product_images as $product_image) {
			...
			$this->data['product_images'][] = array(
				'image'      => $image,
				'thumb'      => $this->model_tool_image->onesize($image, 100, 100),
				'sort_order' => $product_image['sort_order']
			);
		}

		$this->data['no_image'] = $this->model_tool_image->onesize('no_image.jpg', 100, 100);
		...
		foreach ($product_details as $product_detail) {
			...
			$this->data['product_details'][] = array(
				...
				'menu' 		 					=> $product_detail['menu'],
				'image'      					=> $image,
				'thumb'      					=> $this->model_tool_image->cropsize($image, 100, 100),
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'included' 	 => $this->replace_tag(html_entity_decode($result['included'], ENT_QUOTES, 'UTF-8')),
                    'notincluded' => $this->replace_tag(html_entity_decode($result['notincluded'], ENT_QUOTES, 'UTF-8')),
                    'terms' 	 => $this->replace_tag(html_entity_decode($result['terms'], ENT_QUOTES, 'UTF-8')),	
					'start_time' => $result['start_time'],
					'meeting' 	 => $this->replace_tag(html_entity_decode($result['meeting'], ENT_QUOTES, 'UTF-8')),	
					'model'      => $result['model'],
					'option'     => $option_data,
					'price'      => $result['price']
				);	
