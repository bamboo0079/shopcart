...
class ControllerCatalogOption extends Controller {
	...
	private $class_option = array(0 => 'Tour',1 => 'Vé Máy Bay',2 => 'Phụ thu',3 => 'Vé Tàu');   

	

	private $class_category = array(0 => 'Ngày thường',1 => 'Giỗ tổ',2 => 'Hè'); 
	...
	protected function getList() {
		...
		$option_total = $this->model_catalog_option->getTotalOptions($data);
		...
		foreach ($results as $result) {
			...
			$this->data['options'][] = array(
				...
				'hrefedit'	 => $this->url->link('catalog/option/update', 'token=' . $this->session->data['token'] . '&option_id=' . $result['option_id'] . $url, 'SSL')
		...
		$this->data['class_option'] = $this->class_option;

		

		$this->data['class_category'] = $this->class_category;
	...
	protected function getForm() {
		...
		if (isset($this->request->post['class'])) {
			...
			$this->data['class'] = $option_info['class'];
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($options as $option) {
				...
				if($option['category'] == 0){
					...
					$name = "Ngày Thường - ".(html_entity_decode($option['name'], ENT_QUOTES, 'UTF-8'));
				...
				if($option['category'] == 1){
					...
					$name = "Giỗ Tổ - ".(html_entity_decode($option['name'], ENT_QUOTES, 'UTF-8'));
				...
				if($option['category'] == 2){
					...
					$name = "2/9 - ".(html_entity_decode($option['name'], ENT_QUOTES, 'UTF-8'));
