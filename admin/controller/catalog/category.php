...
class ControllerCatalogCategory extends Controller { 
	...
	protected function getList() {
		...
		$data = array(
			...
			'limit' => 1000 //$this->config->get('config_admin_limit')
		);
		...
		foreach ($results as $result) {
			...
			$keyword = $this->model_catalog_category->getCate($result['category_id']);
			$this->data['categories'][] = array(
				'category_id' => $result['category_id'],
				'name'        => $result['name'],
				'keyword'	  => $keyword['keyword'],
				'sort_order'  => $result['sort_order'],
				'status'     => ($result['status'] ? '' : $this->language->get('text_disabled')),
				'selected'    => isset($this->request->post['selected']) && in_array($result['category_id'], $this->request->post['selected']),
				'action'      => $action,
				'hrefedit'	  => $this->url->link('catalog/category/update', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, 'SSL')
			);
		}
		...
		$pagination->limit = 1000 ;//$this->config->get('config_admin_limit')
	...
	protected function getForm() {
		...
		$this->data['entry_desc_footer'] = $this->language->get('entry_desc_footer');
		...
		if (!isset($this->request->get['category_id'])) {
			...
			$this->data['id'] = $this->request->get['category_id'];
		}
		...
		if (isset($this->request->post['menu'])) {
			...
			$this->data['menu'] = $category_info['menu'];
		} else {
			$this->data['menu'] = 1;
		}
