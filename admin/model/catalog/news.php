...
class ModelCatalogNews extends Model {
	public function addNews($data) {
		...
		foreach ($data['news_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "news_description SET news_id = '" . (int)$news_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', custom_title = '" . $this->db->escape($value['custom_title']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', short_description = '" . $this->db->escape($value['short_description']) . "', description = '" . $this->db->escape($value['description']) . "'");
		}
		...
		if (isset($data['news_image'])) {
			foreach ($data['news_image'] as $news_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "news_image SET news_id = '" . (int)$news_id . "', image = '" . $this->db->escape(html_entity_decode($news_image['image'], ENT_QUOTES, 'UTF-8')) . "', name = '" . $this->db->escape($news_image['name']) . "',sort_order = '" . (int)$news_image['sort_order'] . "'");
			}
		}
		...
		if (isset($data['news_type'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "news_to_type SET news_id = '" . (int)$news_id . "', news_type_id = '" . (int)$data['news_type'] . "'");
		}
		...
		//Update log
		$this->db->query("INSERT INTO `" . DB_PREFIX . "news_log` SET user = '".$this->user->getUserName()."', date = NOW(), news_id = '".(int)$news_id."'");
	...
	public function editNews($news_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "news_to_type WHERE news_id = '" . (int)$news_id . "'");
	...
	public function getNewsDescriptions($news_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_description nd WHERE nd.news_id = '" . (int)$news_id . "'");
	...
	public function getNewsTypes($news_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_to_type WHERE news_id = '" . (int)$news_id . "'");
		return $query->row;
	}
	...
	//LOG UPDATE
	public function getNewsLogs($news_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_log WHERE news_id = '" . (int)$news_id . "'");
