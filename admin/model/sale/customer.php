...
class ModelSaleCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', birthday = '" . $this->db->escape($data['birthday_year']).'-'.$this->db->escape($data['birthday_month']).'-'. $this->db->escape($data['birthday_day']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', customer_type_id = '" . (int)$data['customer_type_id'] . "', status = '" . (int)$data['status'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function getCustomers($data = array()) {
		$sql = "SELECT *, CONCAT(c.lastname, ' ', c.firstname) AS name, cgd.name AS customer_group, ctd.name AS customer_type FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "customer_type_description ctd ON (c.customer_type_id = ctd.customer_type_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_customer_type_id'])) {
			$implode[] = "c.customer_type_id = '" . (int)$data['filter_customer_type_id'] . "'";
		}	
	...
	public function getTotalCustomers($data = array()) {
		...
		if (!empty($data['filter_customer_type_id'])) {
			$implode[] = "customer_type_id = '" . (int)$data['filter_customer_type_id'] . "'";
		}
	...
	//
	public function updateGroupByCId($customer_id,$customer_group_id) {
