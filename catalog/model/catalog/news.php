...
class ModelCatalogNews extends Model {
	...
	public function getNews($news_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *,n.news_id as id, nd.name AS name, n.image, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "news_comment c2 WHERE c2.news_id = n.news_id AND c2.status = '1' GROUP BY c2.news_id) AS total_comments, news_category_id FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) LEFT JOIN " . DB_PREFIX . "news_to_store n2s ON (n.news_id = n2s.news_id) LEFT JOIN " . DB_PREFIX . "news_to_category nc ON (n.news_id = nc.news_id) WHERE n.news_id = '" . (int)$news_id . "' AND nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.status = '1' AND n.date_available <= NOW() AND n2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY n.news_id");	
	...
	public function getNewsType($news_id) {
    ...
    public function checkDisplay($id,$type){
        $query = $this->db->query("SELECT * FROM common_news WHERE category = '".$id."' AND type = '".$type."'");
        return $query->row;
    }
