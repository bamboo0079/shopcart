<?php 
class ControllerPromotionHe extends Controller {
	public function index() {
		$this->language->load('promotion/he');
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => $this->config->get('he_title'),
			'href'      => $this->url->link('promotion/he'),      		
			'separator' => $this->language->get('text_separator')
		);
		$this->document->addLink('http://www.vietfuntravel.com.vn/tour-du-lich-he.html', 'canonical');
		$this->document->setTitle($this->config->get('he_customtitle'));
		$this->document->setDescription($this->config->get('he_metadesc'));
		$this->document->setKeywords($this->config->get('he_metakey'));
		$this->data['heading_title'] = $this->config->get('he_title');
		$this->data['url'] = $this->url->link('promotion/he');
		$this->data['desc'] = html_entity_decode($this->config->get('he_desc'), ENT_QUOTES, 'UTF-8');
		...
		/**********************cats**********************/
		$cat_id_promotion = 116;
		$this->data['cats'] = array();
		$cats = $this->model_catalog_tag->getTags($cat_id_promotion);
		foreach($cats as $item_parent) {
			$parents = $this->model_catalog_tag->getTags($item_parent['tag_id']);
		...
		$product = explode(',', $this->config->get('khmsg1nhe_product'));
		if($product){
		...
		$product = explode(',', $this->config->get('khmsg3nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmpt1nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmpt3nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmdn1nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmdn3nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmhn1nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmhn3nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmpq1nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		$product = explode(',', $this->config->get('khmpq3nhe_product'));
		if($product){
			foreach($product as $product_id){
				$result = $this->model_catalog_product->getProduct($product_id);
				if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/promotion/he.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/promotion/he.tpl';
		} else {
			$this->template = 'default/template/promotion/he.tpl';
		}
		$this->children = array(
			'common/column_left',
			'common/column_right',
