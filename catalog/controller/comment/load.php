...
class ControllerCommentLoad extends Controller {
	public function index() {
		...
		$results = $this->model_catalog_comment->getCommentsByUrlParent($this->request->get['url'], ($page - 1) * 5, 5);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/comment/comment.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/comment/comment.tpl';
		} else {
			$this->template = 'default/template/comment/comment.tpl';
		}
