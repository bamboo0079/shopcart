...
class ControllerCheckoutCart extends Controller {
	...
	public function add() {
		...
		if ($product_info) {			
			...
			if (isset($this->request->post['dayment'])) {
				$day_payment = $this->request->post['dayment'];
			} else {
				$day_payment = 0;
			}
					...
					foreach ($newoption as $option_id => $option_values) {
						if (is_array($option_values)) {
							foreach ($option_values as $option_value_id => $quantity) {
								...
								$this->session->data['day_pay'][$this->request->post['product_id']][$datevalue] = $day_payment;/* code by minh event am lich*/
								$option = array();
							}
						} else {
							$this->cart->add($this->request->post['product_id'], $quantity, $option);
