...
class ControllerCheckoutManual extends Controller {
	public function index() {
		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {	
				...
				if ($product_info) {
					...
					if (!isset($json['error']['product']['option'])) {
						...
						if ($option) {
							$dateopt = end($option);
							$newoption = $option;
							foreach ($newoption as $option_id => $option_values) {
								$datevalue = $option_values;
								$dateid = $option_id;
							}
							array_pop($newoption);
							foreach ($newoption as $option_id => $option_values) {
								if (is_array($option_values)) {
									foreach ($option_values as $option_value_id => $quantity) {
										if(isset($dateid)){
											$option[$dateid] = $datevalue;
										}
										$option[$option_id] = array($option_value_id);
			...
			//UPDATE
			$start_time_data = $meeting_data = $included_data = $notincluded_data = $terms_data = array();

			if (isset($this->request->post['order_product'])) {
				foreach ($this->request->post['order_product'] as $order_product) {
					$start_time_data[] = $order_product['start_time'];
					$meeting_data[] = $order_product['meeting'];
					$included_data[] = $order_product['included'];
					$notincluded_data[] = $order_product['notincluded'];
					$terms_data[] = $order_product['terms'];
				}
			}
			if(isset($this->request->post['start_time_input'])){
				$start_time_data = array_merge($start_time_data,(array)$this->request->post['start_time_input']);
			}
			if(isset($this->request->post['meeting_input'])){
				$meeting_data = array_merge($meeting_data,(array)$this->request->post['meeting_input']);
			}
			if(isset($this->request->post['included_input'])){
				$included_data = array_merge($included_data,(array)$this->request->post['included_input']);
			}
			if(isset($this->request->post['notincluded_input'])){
				$notincluded_data = array_merge($notincluded_data,(array)$this->request->post['notincluded_input']);
			}
			if(isset($this->request->post['terms_input'])){
				$terms_data = array_merge($terms_data,(array)$this->request->post['terms_input']);
			}
			//END UPDATE
			...
			foreach ($products as $product) {
				...
				//UPDATE
				if($product_id_save == $product['product_id']){
					$start_time_data = array_merge($start_time_data,(array)$start_time_data[$count-1]);
					$meeting_data = array_merge($meeting_data,(array)$meeting_data[$count-1]);
					$included_data = array_merge($included_data,(array)$included_data[$count-1]);
					$notincluded_data = array_merge($notincluded_data,(array)$notincluded_data[$count-1]);
					$terms_data = array_merge($terms_data,(array)$terms_data[$count-1]);
				}
				//END UPDATE
				$json['order_product'][] = array(
					...
					'start_time'       => $start_time_data[$count],
					'meeting'       => $meeting_data[$count],
					'included'       => $included_data[$count],
					'notincluded'       => $notincluded_data[$count],
					'terms'       => $terms_data[$count],
					//END UPDATE
					'option'     => $option_data,
