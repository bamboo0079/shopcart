...
class ControllerModuleBanner extends Controller {
	protected function index($setting) {
		...
		if(EVENT_ID){		
			$results = $this->model_design_banner->getBanner($setting['banner_id'],date('Y-m-d'));
		...
		$this->data['banners_mobile'] = array();
		
		$results_mobile = $this->model_design_banner->getBannerMobile($setting['banner_id'],date('Y-m-d'));

		foreach ($results_mobile as $result) {
			if (file_exists(DIR_IMAGE . $result['image'])) {
				$this->data['banners_mobile'][] = array(
					'title' => $result['title'],
					'link'  => $result['link'],
