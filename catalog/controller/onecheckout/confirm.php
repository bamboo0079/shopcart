		...
		if (!$json) {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				$this->data['products'][] = array(
					...
					'day_pay'   => $product['day_pay'],/* code by minh event am lich*/
...
//					'total'      => ($version_int < 1540)?$this->currency->format($product['total']):$this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']),
					'total'      => ($version_int < 1540)? $product['total']:$this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity'],
		...
		if (!$json) {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				$product_data[] = array(
					...
					'model'      => $product['model'],
					'included'      => $option_include?$option_include:$product['included'],
