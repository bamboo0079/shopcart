...
class ControllerOneCheckoutAddress extends Controller {
	...
	public function getinfo() {
		...
		if(isset($this->session->data['guest'])){
			$payment_email = $this->session->data['guest']['email'];
			$payment_telephone = $this->session->data['guest']['telephone'];
		}else{
			$result = $this->model_account_customer->getCustomer($this->session->data['customer_id']);
			$payment_email = $result['email'];
			$payment_telephone = $result['telephone'];
		}
		
		
		if(isset($payment_address)) {
			$json['paymentaddress'] = '';
			$json['paymentaddress'] .='<div class="info_payment">';
			$json['paymentaddress'] .= '<p><label>' . $this->language->get('entry_fullname') . '</label><span>' . $payment_address['lastname'].' ' .$payment_address['firstname'] .'</span></p>';
			$json['paymentaddress'] .= '<p><label>' . $this->language->get('entry_email') . '</label><span>' .$payment_email.'</span></p>';
			$json['paymentaddress'] .= '<p><label>' . $this->language->get('entry_telephone') . '</label><span>' .$payment_telephone.'</span></p>';
			$json['paymentaddress'] .= '<p><label>' .$this->language->get('entry_address_1'). '</label><span>';
			if($payment_address['address_1']){
				$json['paymentaddress'] .= $payment_address['address_1']. ', ';
			}
			$json['paymentaddress'] .= $payment_address['zone'] . ', ' . $payment_address['country'].'</span></p>';
			$json['paymentaddress'] .='</div>';
		} else {
			$json['paymentaddress'] = '';
		}
		...
		if (isset($this->session->data['payment_method']['title'])) {
			$json['paymentmethod'] = sprintf($this->language->get('text_payment_method_confirm'),$this->session->data['payment_method']['title']);
		} else {
			$json['paymentmethod'] = '';
		}
		...
		if (isset($this->session->data['comment']) && $this->session->data['comment'] != '') {
			$this->language->load('onecheckout/checkout');
			$json['paymentmethod'] .= '<br /><br /><h2 class="onecheckout-heading onecheckout-note">'.$this->language->get('text_checkout_comment').'</h2>';
