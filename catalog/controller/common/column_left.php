...
class ControllerCommonColumnLeft extends Controller {
    protected function index() {
        ...
        // kiem tra neu ton tai common menu
        for($i=0; $i < count($module_data); $i++){
            if($module_data[$i]['code']=='common_menu' || $module_data[$i]['code']=='common_news'){
                $check_common_menu = $this->getChild('module/'.$module_data[$i]['code'],$module_data[$i]['setting']);
                if(!empty($check_common_menu)){
                    $module = $this->getChild('module/' . $module_data[$i]['code'], $module_data[$i]['setting']);
                    if ($module) {
