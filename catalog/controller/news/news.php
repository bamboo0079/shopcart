...
class ControllerNewsNews extends Controller {
	...
	public function index() {
		...
		if ($news_info) {
			...
			$catenews = $this->model_catalog_news_category->getNewsCategory($news_info['news_category_id']);
			if($catenews){
			$this->data['breadcrumbs'][] = array(
				'text'      => $catenews['name'],
				'href'      => $this->url->link('news/news_category', $url . '&news_category_id=' . $catenews['news_category_id']),
				'separator' => $this->language->get('text_separator')
			);
			}
			...
			$this->document->setTitle($news_info['custom_title']?$news_info['custom_title']:$news_info['name']);
			...
			$this->data['tab_comment'] = sprintf($this->language->get('tab_comment'), $news_info['name']);
			...
			if ($news_info['image']) {
				$this->data['popup'] = $this->model_tool_image->onesize($news_info['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
			} else {
				$this->data['popup'] = '';
			}
			
			if ($news_info['image']) {
				$this->data['thumb'] = $this->model_tool_image->onesize($news_info['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
			} else {
				$this->data['thumb'] = '';
			}
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->onesize($result['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'))
				);
			}
			
						
			$this->data['date_added'] = date('D, '.$this->language->get('date_format_short'), strtotime($news_info['date_added']));			
			...
			$this->data['viewed'] = sprintf($this->language->get('text_viewed'), (int)$news_info['viewed']);
			...
			$results = $this->model_catalog_news->getOtherNews($news_category_info['news_category_id'], $this->request->get['news_id'], 6);
