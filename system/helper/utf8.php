...
function date_to_time($data){//date vietnam
	if($data){
		$date_array = explode('/',$data);
		$date_new = $date_array[1].'/'.$date_array[0].'/'.$date_array[2];
		return $date_new;
	}
}

function datedata_to_date($data){//date vietnam
	if($data){
		$date_array = explode('-',$data);
...
function date_to_time_data($data){//date vietnam
	if($data){
		$date_array = explode('/',$data);
		$date_new = $date_array[2].'-'.$date_array[1].'-'.$date_array[0];
		return $date_new;
...
function cutString($noidung,$num,$appenddot=true) {
	$limit = $num - 1 ;
	$str_tmp = '';
...
function GetLinkByText($string){
	...
	return preg_replace_callback("&\\b$rexProtocol$rexDomain$rexPort$rexPath$rexQuery$rexFragment(?=[?.!,;:\"]?(\s|$))&",
	    'callback', htmlspecialchars($string));
}
